...
**Status:** Pre-audit — active hardening in progress
...
| CC6.1 | Session termination and token revocation | **Not yet implemented.** Design calls for `revoked_tokens` table, logout endpoint in authRoutes, and blacklist check in authPlugin | `backend/src/routes/authRoutes.ts` (planned), `backend/src/plugins/authPlugin.ts` (planned) | **Planned** |
...
| CC6.3 | Logical access — authorization and tenant isolation | Row-Level Security enabled on 9 PostgreSQL tables via migration. `withTenant()` helper sets `app.tenant_id` session variable in transactions. Tenant plugin validates membership before granting access | `backend/prisma/migrations/20260228120000_rls_policies/migration.sql`, `backend/src/db/prisma.ts` (`withTenant()`), `backend/src/plugins/tenantPlugin.ts` | **Partial** |
| CC6.3 | RLS coverage detail | RLS enabled on: `integrations`, `assets`, `jobs`, `distribution_events`, `audit_log`, `kb_documents`, `decision_memos`, `crm_contacts`, `crm_companies`. **Not yet enabled on:** `consent_records`, `data_subject_requests`, `data_breaches` (3 of 12 planned tables still missing). `FORCE ROW LEVEL SECURITY` not yet enabled — Prisma connects as superuser, so RLS is advisory until Phase 3 | `backend/prisma/migrations/20260228120000_rls_policies/migration.sql` | **Partial** |
...
| CC6.6 | System operations — rate limiting | Global IP-based rate limiting: 60 requests/minute via `@fastify/rate-limit`. **Per-tenant rate limiting not yet implemented** — design calls for `rate_limit_buckets` table and `tenantRateLimit.ts` plugin with tiered limits (auth: 10/min, mutation: 30/min, read: 120/min) | `backend/src/server.ts` (line 191) | **Partial** — global only, per-tenant planned |
| CC6.6 | CSRF protection | CSRF plugin exists with double-submit cookie pattern but is **currently disabled** in server.ts. Design calls for rewrite to DB-backed synchronizer token pattern (needed because Vercel-to-Render cross-origin breaks cookie-based CSRF) | `backend/src/plugins/csrfPlugin.ts` (exists but disabled), `backend/src/server.ts` (line 199, commented out) | **Planned** — plugin exists, not active |
| CC6.7 | Change management — transport security | Helmet registered with CSP directives (default-src, script-src, style-src, img-src, connect-src, frame-src, object-src, base-uri). **HSTS not explicitly configured** — Helmet defaults do not enable HSTS. Design calls for `maxAge: 31536000, includeSubDomains: true` and tighter CSP | `backend/src/server.ts` (lines 169-189) | **Partial** — CSP present, HSTS not yet configured |
...
| CC7.2 | Tamper-evident audit logs | **Not yet implemented.** Design calls for hash-chained audit logs: `prev_hash` and `record_hash` columns on `audit_log`, SHA-256 chain per tenant, and verification endpoint at `GET /v1/compliance/audit/verify` | `backend/src/lib/auditChain.ts` (planned), `backend/src/plugins/auditPlugin.ts` (planned modification) | **Planned** |
