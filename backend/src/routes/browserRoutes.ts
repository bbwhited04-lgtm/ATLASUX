...
export const browserRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/sessions", async (req, reply) => {
    ...
    const sessions = await withTenant(tenantId, async (tx) =>
      tx.browserSession.findMany({
        where: { tenantId },
  ...
  app.get("/sessions/:id", async (req, reply) => {
    ...
    return withTenant(tenantId, async (tx) => {
      const session = await tx.browserSession.findFirst({
        where: { id, tenantId },
    ...
    return withTenant(tenantId, async (tx) => {
      ...
      if (session.status === "pending") {
        ...
        const intent = session.intentId
          ? await tx.intent.findUnique({ where: { id: session.intentId } })
          : null;
        ...
        await tx.browserSession.delete({ where: { id } });
