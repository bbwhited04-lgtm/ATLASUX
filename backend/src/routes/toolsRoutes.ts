...
export const toolsRoutes: FastifyPluginAsync = async (app) => {
  ...
  async function addToolToKb(
    ...
    const normalizedAgentSlug = agentId.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
    const toolSlug = toolName.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
    const slug  = `agent/${normalizedAgentSlug}/tool-${toolSlug}`;
    ...
    // ── Policy addendum ─────────────────────────────────────────────────────
    // Aggregate all approved tools for this agent into a single policy addendum
    // KB doc. Uses the `agent/{agentId}/` slug prefix so it's automatically
    // included in the agent's KB pack (getKbContext agent pack).
    await updatePolicyAddendum(tenantId, agentId);
  }

  /**
   * Rebuild the policy addendum KB doc for an agent.
   * Lists every approved tool from tool_proposals so the agent's KB context
   * always reflects its full approved-tool inventory — closing the gap between
