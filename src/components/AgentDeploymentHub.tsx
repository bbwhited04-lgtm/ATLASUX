        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <div className="rounded-xl border border-slate-200 bg-white p-3">
            ...
            <input
              ...
              className="mt-2 w-full rounded-lg bg-white px-3 py-2 text-sm text-slate-900 outline-none ring-1 ring-slate-200 focus:ring-blue-500/30"
            />
          </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            <input
              ...
              className="w-full rounded-lg bg-white px-3 py-2 text-sm text-slate-900 outline-none ring-1 ring-slate-200 focus:ring-blue-500/30"
              placeholder="Task title"
            />
            <textarea
              value={taskDesc}
              onChange={(e) => setTaskDesc(e.target.value)}
              className="h-24 w-full rounded-lg bg-white px-3 py-2 text-sm text-slate-900 outline-none ring-1 ring-slate-200 focus:ring-blue-500/30"
          ...
          <div className="mt-3 space-y-2">
            {tasks.slice(0, 20).map((t) => (
              <div key={t.id} className="rounded-xl border border-slate-200 bg-white p-3">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-slate-900">{t.input?.title ?? t.id}</div>
                  <div className="text-xs text-slate-400">{t.status}</div>
                </div>
                <div className="mt-1 text-xs text-slate-600/80">{t.input?.description ?? ""}</div>
          ...
          <div className="mt-3 space-y-2">
            {outbox.slice(0, 20).map((j) => (
              <div key={j.id} className="rounded-xl border border-slate-200 bg-white p-3">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-slate-900">{j.input?.subject ?? "Email"}</div>
                  <div className="text-xs text-slate-400">{j.status}</div>
                </div>
                <div className="mt-1 text-xs text-slate-600/80">To: {j.input?.to}</div>
                <div className="mt-1 text-xs text-slate-600/80">From agent: {j.input?.fromAgent}</div>
              </div>
            ))}
