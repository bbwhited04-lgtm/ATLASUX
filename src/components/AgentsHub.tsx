  ...
  return (
    <div className="rounded-2xl border border-cyan-500/10 bg-slate-950/40 p-5 shadow-lg shadow-cyan-500/5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-sm text-slate-400">{agent.tier}</div>
          <div className="text-lg font-semibold text-white">{agent.name}</div>
          <div className="text-sm text-cyan-300/90">{agent.title}</div>
        </div>
  ...
  return (
    <div className="p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="relative w-full md:w-[420px]">
          ...
          <input
            ...
            className="w-full rounded-xl bg-slate-950/40 border border-cyan-500/10 pl-10 pr-3 py-2 text-sm text-slate-200 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/30"
          />
        </div>
      </div>

      <div className="mt-6 rounded-2xl border border-cyan-500/10 bg-slate-950/30 p-4">
      ...
      <Tabs.Root defaultValue="atlas" className="mt-6">
        <Tabs.List className="flex flex-wrap gap-2 rounded-2xl border border-cyan-500/10 bg-slate-950/30 p-2">
          <Tabs.Trigger value="atlas" className="px-4 py-2 text-sm rounded-xl data-[state=active]:bg-cyan-500/20 data-[state=active]:text-cyan-200 text-slate-300 hover:bg-slate-800/50">
            <Briefcase className="inline h-4 w-4 mr-2" /> Atlas + Staff
          </Tabs.Trigger>
          <Tabs.Trigger value="binky" className="px-4 py-2 text-sm rounded-xl data-[state=active]:bg-cyan-500/20 data-[state=active]:text-cyan-200 text-slate-300 hover:bg-slate-800/50">
            <Shield className="inline h-4 w-4 mr-2" /> Binky + Crew
          </Tabs.Trigger>
          <Tabs.Trigger value="board" className="px-4 py-2 text-sm rounded-xl data-[state=active]:bg-cyan-500/20 data-[state=active]:text-cyan-200 text-slate-300 hover:bg-slate-800/50">
            <Crown className="inline h-4 w-4 mr-2" /> Board & Governors
          </Tabs.Trigger>
          <Tabs.Trigger value="all" className="px-4 py-2 text-sm rounded-xl data-[state=active]:bg-cyan-500/20 data-[state=active]:text-cyan-200 text-slate-300 hover:bg-slate-800/50">
            All
          </Tabs.Trigger>
        </Tabs.List>
