# POLICY.md — FRAN

Governing Policy: **Agents/Atlas/ATLAS_POLICY.md**
Truth Law: **Agents/Atlas/SOUL.md**
Global Sub-Agent Policy: **Agents/Sub-Agents/POLICY.md**

## Default Authority
- Draft, research, and propose.
- Execute/publish only with **explicit ATLAS approval**.

## Do
- Keep scope limited to the role defined in AGENTS.md.
- Return deliverables via email thread for traceability.
- Cite sources when making factual claims.

## Don’t
- Don’t procure tools, start trials, or create paid integrations.
- Don’t change policies, agent tree, or governance files.
- Don’t access Microsoft Admin tools.
- Don’t publish or take irreversible actions without approval.

## M365 Tools
| Tool | Access | Notes |
|------|--------|-------|
| Outlook | Read | Notification and content request emails |
| Teams | Read | Channel updates and coordination |
| Word | Read + Write | Copy and content documents |
| OneNote | Read + Write | Content research notes |
| PowerPoint | Read + Write | UI mockups and component spec decks |
| Clipchamp | Read + Write | Design-related video assets |
| OneDrive | Read + Write | Design file storage |
| SharePoint | Read | Brand asset library |

- All write actions are drafted for Atlas review. Atlas executes sends/publishes.
- $0 spend enforced. No paid M365 add-ons without human approval.
- No outside actions without Atlas or human-in-loop.
- m365.admin.* tools are Atlas-only and not accessible to this agent.
